...
def fmt_price(value: Any, currency: str = "") -> str:
    ...
    return f"{v:,.2f}".replace(",", "_").replace(".", ",").replace("_", ".") + (f" {cur}" if cur else "")
...
def exchange_prefix(info: dict[str, Any], ticker: str) -> str:
    exch = str(info.get("exchange") or "").upper()
    full = str(info.get("fullExchangeName") or "").lower()
    if exch in EXCHANGE_MAP:
        return EXCHANGE_MAP[exch]
    if "nasdaq" in full:
        return "NASDAQ"
    if "nyse" in full or "new york" in full:
        return "NYSE"
    if "american" in full or "amex" in full:
        return "AMEX"
...
def enrich_item(item: dict[str, str]) -> dict[str, str]:
    out = dict(item)
    yahoo = str(out.get("yahoo") or out.get("ticker") or "").strip().upper()
    info = get_info(yahoo)
    out["name"] = str(info.get("longName") or info.get("shortName") or info.get("displayName") or out.get("name") or yahoo)
    if not out.get("tv"):
        prefix = exchange_prefix(info, yahoo)
        tv_ticker = yahoo.replace(".MI", "") if prefix == "MIL" else yahoo.replace("-", ".")
        out["tv"] = f"{prefix}:{tv_ticker}"
    out["exchange"] = str(info.get("exchange") or "")
    out["full_exchange_name"] = str(info.get("fullExchangeName") or "")
...
def technical_metrics(item: dict[str, str]) -> dict[str, Any]:
    ...
    row: dict[str, Any] = {
        ...
        "exchange": item.get("exchange", ""), "full_exchange_name": item.get("full_exchange_name", ""), "sector": item.get("sector", ""), "industry": item.get("industry", ""),
        ...
        "gap_points": None, "below_sma200w": False, "orange_zone": False, "in_zone": False,
        "weekly_rows": 0, "last_weekly_date": "", "momentum_26w_pct": None, "momentum_52w_pct": None, "drawdown_52w_pct": None, "weekly_vol_52w_pct": None, "error": "",
    }
    try:
        ...
        row["weekly_rows"] = len(weekly)
        idx = weekly.index[-1]
        row["last_weekly_date"] = idx.strftime("%Y-%m-%d") if hasattr(idx, "strftime") else str(idx)
...
def fundamentals(symbol: str, last_price: Any) -> dict[str, Any]:
    keys = [
        "market_cap", "forward_pe", "trailing_pe", "peg_ratio", "price_to_sales", "enterprise_to_ebitda", "free_cashflow", "fcf_yield_pct",
        "return_on_equity_pct", "gross_margin_pct", "operating_margin_pct", "profit_margin_pct", "debt_to_equity", "revenue_growth_pct",
        "earnings_growth_pct", "beta", "target_mean_price", "target_upside_pct", "recommendation_mean",
    ]
    out = {k: None for k in keys}
    out["fundamental_symbol"] = symbol
    out["info_error"] = ""
    try:
        ...
        out["enterprise_to_ebitda"] = safe_float(get_info_value(info, "enterpriseToEbitda"))
...
def score_technical(row: dict[str, Any]) -> tuple[float, list[str]]:
    ...
    if gap is not None:
        ...
        if gap <= 5:
            notes.append("gap storico stretto")
...
def find_zone_range(record: dict[str, Any], threshold: float) -> dict[str, Any]:
    ...
    lower_pct = -MAX_SIMULATED_DISCOUNT_PCT
    upper_pct = MAX_SIMULATED_DISCOUNT_PCT
    steps = int((upper_pct - lower_pct) / SIMULATION_STEP_PCT)
    for i in range(steps + 1):
        pct = lower_pct + i * SIMULATION_STEP_PCT
    ...
    return {"status": "range", "low": low, "high": high, "active": active}
...
def format_zone_range(record: dict[str, Any], key: str) -> str:
    ...
    if status == "range":
        low = fmt_price(data.get("low"), currency)
        high = fmt_price(data.get("high"), currency)
...
def build_record(item: dict[str, str]) -> dict[str, Any]:
    ...
    tech = technical_metrics(enriched)
    fund = fundamentals(tech.get("yahoo", enriched.get("yahoo", "")), tech.get("last_price"))
    score = compute_score(tech, fund)
    record: dict[str, Any] = {}
    record.update(tech)
    record.update(fund)
    record.update(score)
    record["tradingview_url"] = tv_chart_url(str(record.get("tv") or ""))
...
def scan_symbols(limit: int | None = None) -> list[dict[str, Any]]:
    ...
    return sorted(records, key=lambda row: safe_float(row.get("score_total")) or -1, reverse=True)


def scan_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    top = records[0] if records else {}
    buy_strong = [r for r in records if (safe_float(r.get("score_total")) or 0) >= BUY_ZONE_THRESHOLD]
    orange = [r for r in records if bool(r.get("orange_zone"))]
    errors = [r for r in records if str(r.get("error") or "").strip()]
    return {
        "count": len(records),
        "top_ticker": top.get("ticker", "-"),
        "top_score": top.get("score_total"),
        "buy_strong_count": len(buy_strong),
