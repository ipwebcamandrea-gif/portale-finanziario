...
ALLOCATION_COLORS = [
    ...
    "#84cc16", "#f97316", "#14b8a6", "#e879f9", "#60a5fa", "#facc15",
]
...
def create_position_donut(position_allocation, total_label: str):
    fig = px.pie(
        ...
        custom_data=["ticker", "weight_pct", "value_eur"],
    )
    fig.update_traces(
        textposition="inside",
        textinfo="percent",
        hovertemplate="<b>%{label}</b><br>Ticker: %{customdata[0]}<br>Peso: %{customdata[1]:.2f}%<br>Valore: %{customdata[2]:,.2f} EUR<extra></extra>",
...
def create_position_bar(position_allocation):
    fig = px.bar(
        ...
        color="titolo",
        color_discrete_sequence=ALLOCATION_COLORS,
        custom_data=["ticker", "value_eur"],
        text="weight_pct",
    )
    fig.update_traces(
        texttemplate="%{text:.2f}%",
        textposition="outside",
        hovertemplate="<b>%{y}</b><br>Ticker: %{customdata[0]}<br>Peso: %{x:.2f}%<br>Valore: %{customdata[1]:,.2f} EUR<extra></extra>",
    )
...
def create_group_bar(group_df, group_col: str, title: str):
    ...
    fig.update_traces(
        texttemplate="%{text:.2f}%",
        textposition="outside",
        hovertemplate="<b>%{y}</b><br>Peso: %{x:.2f}%<br>Valore: %{customdata[0]:,.2f} EUR<extra></extra>",
    )
