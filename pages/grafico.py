...
def calcola_macd(serie_close):
    ema_fast = serie_close.ewm(
        span=MACD_FAST,
        adjust=False
    ).mean()

    ema_slow = serie_close.ewm(
        span=MACD_SLOW,
        adjust=False
    ).mean()
...
def prepara_indicatori_weekly(data):
    ...
    data_plot["WMA21W"] = calcola_wma(
        data_plot["Close"],
        WMA_21
    )

    data_plot["WMA50W"] = calcola_wma(
        data_plot["Close"],
        WMA_50
    )

    data_plot["WMA200W"] = calcola_wma(
        data_plot["Close"],
        WMA_200
    )

    data_plot["EMA200W"] = data_plot["Close"].ewm(
        span=EMA_200,
        adjust=False
    ).mean()

    data_plot["SMA200W"] = data_plot["Close"].rolling(
        SMA_200
    ).mean()
    ...
    macd, macd_signal, macd_hist = calcola_macd(
        data_plot["Close"]
    )
...
if errore_download:
    st.warning(
        "Yahoo Finance/YFinance ha limitato o interrotto temporaneamente "
        "la richiesta. Riprova tra qualche minuto."
    )

if data.empty:
    st.error(
        "Non sono stati trovati dati validi per il ticker "
        + ticker
...
st.download_button(
    ...
    data=csv_data,
    file_name=ticker + "_weekly_10y.csv",
    mime="text/csv"
)
