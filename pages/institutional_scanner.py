...
def fib_panel(r,cur):
    ...
    html+=zone('Fib First Buy Area',f"{fmt_price(r.get('fib_first_buy_low'),cur)} - {fmt_price(r.get('fib_first_buy_high'),cur)}",'institutional-fib-zone-first')
    html+=zone('Fib Buy Area',f"{fmt_price(r.get('fib_buy_low'),cur)} - {fmt_price(r.get('fib_buy_high'),cur)}",'institutional-fib-zone-buy')
    html+=zone('Fib Strong Buy Area',f"{fmt_price(r.get('fib_strong_low'),cur)} - {fmt_price(r.get('fib_strong_high'),cur)}",'institutional-fib-zone-strong')
    html+=f'</div><div class="institutional-fib-status"><span>Stato prezzo Fib</span><strong>{escape(str(r.get("fib_status") or ""))}</strong></div></div>'
    return html
def render_card(r,i):
    ...
    html=f'<div class="institutional-card {"institutional-card-buy" if lab!="Monitor tecnico" else ""}"><div class="institutional-card-header"><div class="institutional-rank">#{i}</div><div class="institutional-title-wrap"><div class="institutional-ticker">{escape(str(r.get("ticker") or ""))}</div><div class="institutional-name">{escape(str(r.get("name") or ""))}</div></div><div class="institutional-label {"institutional-label-buy" if lab!="Monitor tecnico" else ""}">● {escape(lab)}</div></div>'
...
if errs:
    with st.expander(f"⚠️ Titoli con dati tecnici incompleti ({len(errs)})", expanded=False):
