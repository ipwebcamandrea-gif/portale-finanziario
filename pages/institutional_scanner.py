...
from utils.institutional_scanner import fmt_gap_points, fmt_num, fmt_pct, fmt_price, safe_float, scan_summary, scan_symbols
...
def label_class(label: str) -> str:
    if label == "Strong Buy Zone": return "institutional-label institutional-label-strong"
    if label == "Buy Zone": return "institutional-label institutional-label-buy"
    return "institutional-label"

def card_class(label: str) -> str:
    if label == "Strong Buy Zone": return "institutional-card institutional-card-strong"
    if label == "Buy Zone": return "institutional-card institutional-card-buy"
    return "institutional-card"

def label_icon(label: str) -> str:
    if label == "Strong Buy Zone": return "🔥"
    if label == "Buy Zone": return "🟢"
...
def render_card(record: dict, rank: int) -> str:
    ticker = escape(str(record.get("ticker") or "")); name = escape(str(record.get("name") or ""))
    label = str(record.get("score_label") or "Monitor"); currency = str(record.get("currency") or "").upper()
    score = fmt_num(record.get("score_total"), 1); price = fmt_price(record.get("last_price"), currency)
    daily = fmt_pct(record.get("daily_change_pct"), 2); daily_class = value_class(record.get("daily_change_pct"))
    tv_url = escape(str(record.get("tradingview_url") or "#"), quote=True); notes = escape(str(record.get("score_notes") or "-") or "-")
    orange_box = orange_metric_box_class(record); error = str(record.get("error") or "").strip()
    card = (
        f'<div class="{card_class(label)}"><div class="institutional-card-header">'
        f'<div class="institutional-rank">#{rank}</div><div class="institutional-title-wrap"><div class="institutional-ticker">{ticker}</div><div class="institutional-name">{name}</div></div>'
        f'<div class="{label_class(label)}">{label_icon(label)} {escape(label)}</div></div>'
        '<div class="institutional-price-score-row"><div class="institutional-price-box"><div class="institutional-mini-label">Prezzo attuale</div>'
        f'<div class="institutional-price-value">{escape(price)}</div><div class="institutional-daily {daily_class}">Daily {escape(daily)}</div></div>'
        f'<div class="institutional-score-box"><div class="institutional-mini-label">Score</div><div class="institutional-score-value">{escape(score)}</div><div class="institutional-score-suffix">/100</div></div></div>'
        ...
        + '</div><div class="institutional-range-panel"><div class="institutional-range-title">Range operativo stimato</div><div class="institutional-range-grid">'
        ...
        f'</div></div><div class="institutional-notes">Motivi: {notes}</div><div class="institutional-card-actions"><a href="{tv_url}" target="_blank" rel="noopener noreferrer">Apri TradingView →</a></div>'
    )
    if error:
...
st.markdown('<div class="institutional-disclaimer">Nota: output informativo e sperimentale, non raccomandazione finanziaria. Lo score combina dati tecnici/fondamentali da yfinance e simulazioni sul prezzo.</div>', unsafe_allow_html=True)
...
cols = st.columns(4)
for col, (label, value, note) in zip(cols, [("Titoli analizzati", str(summary.get("count", 0)), "Lista Mega Cap USA/ETF"), ("Top score", fmt_num(summary.get("top_score"), 1), str(summary.get("top_ticker") or "-")), ("Buy/Strong", str(summary.get("buy_strong_count", 0)), "Score >= 65"), ("Area arancione", str(summary.get("orange_count", 0)), "SMA200W + storico")]):
    with col:
...
if not records:
    ...
    st.markdown('<div class="institutional-grid">' + ''.join(render_card(r, i) for i, r in enumerate(records[:12], 1)) + '</div>', unsafe_allow_html=True)
