...
def metric_html(label: str, value: str, css_class: str = "") -> str:
    css = f" institutional-metric-value {css_class}" if css_class else "institutional-metric-value"
    return (
        '<div class="institutional-metric">'
        f'<div class="institutional-mini-label">{escape(label)}</div>'
        f'<div class="{css}">{escape(value)}</div>'
        '</div>'
    )
...
def render_card(record: dict, rank: int) -> str:
    ...
    card_html = (
        ...
        '</div>'
        f'<div class="{label_class(label)}">{label_icon(label)} {label_text}</div>'
        '</div>'
        ...
        + metric_html("SMA200W", fmt_price(record.get("sma200w"), currency))
        + metric_html("Distanza", fmt_pct(record.get("dist_pct"), 2), value_class(record.get("dist_pct")))
        + metric_html("Area arancione", "SI" if record.get("orange_zone") else "NO")
        + metric_html("Gap storico", fmt_pct(record.get("gap_points"), 1))
        ...
        '<div class="institutional-range-box institutional-range-buy">'
        '<div class="institutional-range-label">Buy Zone</div>'
        f'<div class="institutional-range-value">{escape(str(record.get("buy_zone_text") or "dati insufficienti"))}</div>'
        '</div>'
        '<div class="institutional-range-box institutional-range-strong">'
        '<div class="institutional-range-label">Strong Buy Zone</div>'
        f'<div class="institutional-range-value">{escape(str(record.get("strong_buy_zone_text") or "dati insufficienti"))}</div>'
        '</div>'
        '</div>'
        '</div>'
    ...
    if error:
        card_html += f'<div class="institutional-error-box">Dato incompleto: {escape(error)}</div>'
    card_html += '</div>'
    return card_html
...
st.markdown(
    ...
    'Nota: output informativo e sperimentale, non raccomandazione finanziaria. '
    'Lo score combina dati tecnici/fondamentali da yfinance e simulazioni sul prezzo; verifica sempre i dati prima di decisioni operative.'
    '</div>',
...
if not top_records:
    ...
    html_cards = '<div class="institutional-grid">' + ''.join(render_card(record, idx) for idx, record in enumerate(top_records, 1)) + '</div>'
