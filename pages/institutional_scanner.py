...
def metric(l,v,c="",b=""):
    return f'<div class="institutional-metric {b}"><div class="institutional-mini-label">{escape(l)}</div><div class="institutional-metric-value {c}">{escape(v)}</div></div>'
def zone(l,v,c): return f'<div class="institutional-fib-zone {c}"><div class="institutional-fib-zone-label">{escape(l)}</div><div class="institutional-fib-zone-value">{escape(v)}</div></div>'
def fib_panel(r,cur):
    if not r.get('fib_available'): return '<div class="institutional-fib-panel"><div class="institutional-fib-title">Fibonacci W automatico - Recent Primary Swing</div><div class="institutional-fib-missing">Non disponibile</div></div>'
    m=safe_float(r.get('fib_marker_pct')); style=f"left:{m:.1f}%;" if m is not None else "left:0%;"
    html='<div class="institutional-fib-panel"><div class="institutional-fib-title">Fibonacci W automatico - Recent Primary Swing</div>'
    html+=f'<div class="institutional-fib-swing">Swing recente automatico - <span class="institutional-negative">Min {escape(fmt_price(r.get("fib_low"),cur))} ({escape(str(r.get("fib_low_date") or "N/D"))})</span> - <span class="institutional-positive">Max {escape(fmt_price(r.get("fib_high"),cur))} ({escape(str(r.get("fib_high_date") or "N/D"))})</span></div><div class="institutional-fib-levels">'
    ...
    html+=f'</div><div class="institutional-fib-rail"><div class="institutional-fib-segment-first">FIRST BUY</div><div class="institutional-fib-segment-buy">BUY</div><div class="institutional-fib-segment-strong">STRONG</div><div class="institutional-fib-marker" style="{style}"><span>Prezzo</span></div></div><div class="institutional-fib-zones">'
    ...
    return html
def card(r,i):
    cur=str(r.get('currency') or '').upper(); lab=str(r.get('technical_label') or 'Monitor tecnico'); orange='institutional-metric-orange' if r.get('orange_zone') else ''; tv=escape(str(r.get('tradingview_url') or '#'),quote=True); lbl='institutional-label-buy' if lab!='Monitor tecnico' else ''; cc='institutional-card-buy' if lab!='Monitor tecnico' else ''
    html=f'<div class="institutional-card {cc}"><div class="institutional-card-header"><div class="institutional-rank">#{i}</div><div class="institutional-title-wrap"><div class="institutional-ticker">{escape(str(r.get("ticker") or ""))}</div><div class="institutional-name">{escape(str(r.get("name") or ""))}</div></div><div class="institutional-label {lbl}">● {escape(lab)}</div></div>'
    html+=f'<div class="institutional-price-status-row"><div class="institutional-price-box"><div class="institutional-mini-label">Prezzo attuale</div><div class="institutional-price-value">{escape(fmt_price(r.get("last_price"),cur))}</div><div class="institutional-daily {cval(r.get("daily_change_pct"))}">Daily {escape(fmt_pct(r.get("daily_change_pct"),2))}</div></div><div class="institutional-status-box"><div class="institutional-mini-label">Stato tecnico</div><div class="institutional-status-value">{escape(lab)}</div><div class="institutional-status-note">SMA200W - minimi storici - Fibonacci W</div></div></div><div class="institutional-section-mini-title">Area SMA200W / Storico</div><div class="institutional-metrics-grid">'
    html+=metric('SMA200W',fmt_price(r.get('sma200w'),cur),b=orange)+metric('Distanza SMA200W',fmt_pct(r.get('dist_pct'),2),cval(r.get('dist_pct')),orange)+metric('Area arancione','SI' if r.get('orange_zone') else 'NO',b=orange)+metric('Scarto da Min W',fmt_pct(r.get('gap_points'),1),b=orange)+metric('Hist Min W',fmt_pct(r.get('hist_min_w_pct'),1),b=orange)+metric('MinW Low',f"{fmt_price(r.get('hist_min_w_low'),cur)} ({r.get('hist_min_w_date') or 'N/D'})",b=orange)+metric('Eq oggi MinW',fmt_price(r.get('hist_min_equivalent'),cur),b=orange)+metric('Hist Max W',fmt_pct(r.get('hist_max_w_pct'),1),b=orange)
    return html+'</div>'+fib_panel(r,cur)+f'<div class="institutional-card-actions"><a href="{tv}" target="_blank" rel="noopener noreferrer">Apri TradingView</a></div></div>'
render_standard_page_header(title="Scanner SMA200W / Fibonacci", subtitle="Scanner tecnico weekly: SMA200W, minimi storici sotto media e Recent Primary Swing Fibonacci W.", toggle_label="📱 Vista compatta", toggle_key="institutional_compact_mode", toggle_default=True, refresh_key="institutional_header_refresh", back_key="institutional_header_back", refresh_callback=refresh_scan)
