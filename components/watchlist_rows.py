...
def _render_text_lines_html(value):
...
def _cell_html_with_subvalue(label, value, subvalue="", css_class="tv-cell-value"):
    ...
    if subvalue:
        html += f'<div class="tv-cell-subvalue">{_render_text_lines_html(subvalue)}</div>'
    return html
...
def _sma200_history_label(metrics):
    return "\n".join(_sma200_history_lines(metrics))


def _compact_sma200_history_html(metrics):
    history_lines = _sma200_history_lines(metrics)
    if not history_lines:
        return ""
    history_html = "<br>".join(escape(line) for line in history_lines)
    return (
        '<span class="tv-compact-separator">•</span>'
        f'<span class="tv-sma-estimate">{history_html}</span>'
    )
